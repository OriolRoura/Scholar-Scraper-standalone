...
class CaptchaDetectedException(Exception):
    """Captcha was detected and could not be solved within the configured timeout."""
...
class ProxyGenerator(object):
    ...
    def _get_chrome_webdriver(self):
        ...
        # Allow overriding headless mode via env var for manual captcha solving.
        # By default preserve existing behaviour (headless) unless the user sets
        # SCHOLAR_WEBDRIVER_HEADLESS to '0' or 'false'.
        try:
            headless_env = os.environ.get('SCHOLAR_WEBDRIVER_HEADLESS', '1').lower()
    ...
    def _handle_captcha2(self, url):
        # First: give an automatic (headless) webdriver attempt a short time
        # to resolve the captcha. If that fails, fall back to opening a
        # visible browser via the repository's `manual_solve` helper once.
        # If manual solving fails, raise CaptchaDetectedException so callers
        # (the top-level scraper) can save partial results and exit.

        auto_timeout = 30

        try:
            # Use webdriver to attempt automatic resolution
            wd = self._get_webdriver()
            cur_host = urlparse(wd.current_url).hostname
            # inject cookies for current host
            ...
            log_interval = 5
            elapsed = 0
            while elapsed < auto_timeout:
                try:
                    WebDriverWait(wd, log_interval).until_not(lambda drv: self._webdriver_has_captcha())
                    # solved
                    break
                except TimeoutException:
                    elapsed += log_interval
                    self.logger.info(f"Automatic captcha attempt: waiting {elapsed}/{auto_timeout}s")
                    ...
                    self.logger.info(f"WebDriver error during automatic captcha attempt: {e}")
                    break
                except Exception as e:
                    self.logger.info(f"Unhandled exception during automatic captcha attempt: {e}")
                    break
            else:
                self.logger.info(f"Automatic captcha attempt timed out after {auto_timeout}s")

            # If automatic cleared the captcha, copy cookies back and return session
            try:
                if not self._webdriver_has_captcha():
                    for cookie in wd.get_cookies():
                        ...
                        try:
                            ...
                            try:
                                ck = requests.cookies.create_cookie(name=cookie.get('name'), value=cookie.get('value'), domain=cookie.get('domain'), path=cookie.get('path', '/'))
                                self._session.cookies.set_cookie(ck)
                            except Exception:
                                pass
                    self.logger.info("Automatic captcha attempt succeeded")
                    return self._session
            except Exception:
                pass
        except Exception as e:
            self.logger.info(f"Automatic captcha attempt aborted: {e}")

        # Automatic attempt failed or timed out: try manual solve once
        try:
            from gs_library.selenium_on_demand import manual_solve
        except Exception:
            manual_solve = None

        if manual_solve is None:
            msg = "CAPTCHA_DETECTED: automatic solver failed and manual solver unavailable."
            self.logger.warning(msg)
            raise CaptchaDetectedException(msg)
        ...
        try:
            try:
                ...
                persist_path = os.path.join(cache_dir, 'last_solved_session.json')
            except Exception:
                persist_path = None

            cookies, local = manual_solve(url, wait_indefinite=True, persist_cookies_path=persist_path)
            if not cookies:
                raise Exception('manual_solve did not return cookies')
            ...
            for c in cookies:
                ...
                if name is None:
                    continue
                try:
                    ck = requests.cookies.create_cookie(name=name, value=val, domain=domain, path=path)
                    self._session.cookies.set_cookie(ck)
                except Exception:
                    try:
            ...
            self.logger.info('Manual solve succeeded and cookies injected.')
            return self._session
        except Exception as e:
            msg = f"CAPTCHA_DETECTED: manual solver failed ({e})."
            self.logger.warning(msg)
            # Give up: bubble up dedicated exception so scraper can save partials and exit
            raise CaptchaDetectedException(msg)
