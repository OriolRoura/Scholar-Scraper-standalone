...
from ._proxy_generator import ProxyGenerator, MaxTriesExceededException, DOSException, CaptchaDetectedException
...
class Navigator(object, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self._TIMEOUT = 5
        self._max_retries = 1
        # Variable wait configuration: can be tuned via environment variables
        # Defaults chosen to add small random delays and occasional jitter.
        try:
            # Default wait between requests: 6-8 seconds
            self.min_delay = float(os.getenv('SCHOLAR_MIN_DELAY', '6'))
            self.max_delay = float(os.getenv('SCHOLAR_MAX_DELAY', '8'))
            self.jitter_probability = float(os.getenv('SCHOLAR_JITTER_PROB', '0.2'))
            # Jitter adds another 6-8 seconds when applied
            self.jitter_min = float(os.getenv('SCHOLAR_JITTER_MIN', '6'))
            self.jitter_max = float(os.getenv('SCHOLAR_JITTER_MAX', '8'))
        except Exception:
            # Fallback sensible defaults
            self.min_delay = 6.0
            self.max_delay = 8.0
            self.jitter_probability = 0.2
            self.jitter_min = 6.0
            self.jitter_max = 8.0
    ...
    def _get_page(self, pagerequest: str, premium: bool = False) -> str:
        ...
        while tries < self._max_retries:
            try:
                # Variable, human-like wait before each request
                delay = random.uniform(self.min_delay, self.max_delay)
                # Occasional long jitter to mimic human breaks
                if random.random() < self.jitter_probability:
                    extra = random.uniform(self.jitter_min, self.jitter_max)
                    delay += extra
                    self.logger.info(f'Applying jitter: extra {extra:.2f}s (total delay {delay:.2f}s)')
                time.sleep(delay)
                ...
                # Bubble up captcha detection so higher-level code can stop immediately
                self.logger.info("Captcha could not be solved in webdriver. Raising to stop retries.")
                raise
